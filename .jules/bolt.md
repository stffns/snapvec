...
## 2024-05-18 - Fast single-vector L2 norm in pure NumPy
...
**Action:** Always prefer `np.sqrt(np.inner(q, q))` over `np.linalg.norm(q)` when computing single-vector norms in NumPy to eliminate dispatch overhead and improve execution speed.
