...
## 2024-05-19 - Fast L2 norm for 1D arrays
...
**Action:** Always prefer `np.sqrt(np.vdot(q, q))` over `np.linalg.norm(q)` when calculating L2 norms for 1D NumPy arrays to avoid the dispatching and type-checking overhead.
