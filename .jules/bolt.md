...
## 2024-05-20 - Fast 1D L2 norm computation in pure NumPy
...
**Action:** Always prefer `np.sqrt(np.vdot(q, q))` over `np.linalg.norm(q)` when computing L2 norms of 1D numpy arrays to eliminate dispatch overhead and improve execution speed.
