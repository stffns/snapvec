...
class PQSnapIndex(FreezableIndex):
    ...
    def _preprocess_single(
        ...
        # Optimized: np.sqrt(np.vdot(q, q)) is ~1.6x faster than np.linalg.norm(q) for 1D arrays
        ...
        if self.use_rht:
            ...
            # Optimized: np.sqrt(np.vdot(rot, rot)) is ~1.6x faster than np.linalg.norm(rot)
    ...
    def search(
        ...
        # Optimized: np.sqrt(np.vdot(q, q)) is ~1.6x faster than np.linalg.norm(q)
