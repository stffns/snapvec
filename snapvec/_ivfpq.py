...
class IVFPQSnapIndex(FreezableIndex):
    ...
    def _preprocess_single(self, q: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        # Optimized: np.sqrt(np.inner(q, q)) is ~1.5x faster than np.linalg.norm(q) for 1D arrays
        ...
        if self.use_rht:
            ...
            # Optimized: np.sqrt(np.inner(..., ...)) is ~1.5x faster than np.linalg.norm for 1D arrays
